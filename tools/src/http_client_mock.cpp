 ...
 *   a-load <name> [json] — load adapter on all nodes (POST /api/adapters/:name)
 *   a-unload <name>    — unload adapter on all nodes (DELETE /api/adapters/:name)
 *   rp                 — list resource providers on all nodes (GET /api/resource-providers)
 *   rpav               — list available resource providers (GET /api/resource-providers/available)
 *   rp-load <name> [json] — load resource provider (POST /api/resource-providers/:name)
 *   rp-unload <name>   — unload resource provider (DELETE /api/resource-providers/:name)
 ...
 * Auth: validates X-API-Key header (mock key printed on startup).
 */
    ...
    forEachNode(
        ...
        [](httplib::Client& client, const RegisteredNode& node) {
            ...
            return handleResponse(
                ...
                node.node_id,
                [&](const nlohmann::json& resp) {
                    std::cout << "[Mock] Resource providers on " << node.node_id << ":\n"
                        << resp.dump(2) << "\n";
                }
            );
        }
    ...
    forEachNode(
        ...
        [](httplib::Client& client, const RegisteredNode& node) {
            ...
            return handleResponse(
                ...
                node.node_id,
                [&](const nlohmann::json& resp) {
                    std::cout << "[Mock] Available resource providers on " << node.node_id << ":\n"
                        << resp.dump(2) << "\n";
                }
            );
        }
    ...
    for(auto& sol_dir : args.solutions) {
        ...
        try {
            ...
            if(args.memory_limit_mb >= 0) {
                request["memoryLimitMb"] = args.memory_limit_mb;
            }
...
int main(int argc, char** argv) {
    if(argc > 1) {
        try { g_state.listen_port = std::stoi(argv[1]); } catch(const std::exception&) {
    ...
    // X-API-Key auth middleware — accept any key (mock: no validation)
    svr.set_pre_routing_handler(
        [](const httplib::Request& req, httplib::Response&) -> httplib::Server::HandlerResponse {
            auto it = req.headers.find("X-API-Key");
            if(it != req.headers.end()) {
                std::cout << "[Mock] X-API-Key: " << it->second << "\n";
            }
        ...
        [](const httplib::Request& req, httplib::Response& res) {
            try {
                ...
                std::cout << "[Mock] REGISTER: node_id=" << node_id
                    ...
                    << " auth_token=" << auth_token << " -> CONFIRMED\n";
    ...
    std::cout << "[Mock] Commands: s=status, l=list, qs=queue status, a=adapters, av=available,\n"
        << "                a-load/a-unload <name>, rp=resource providers, rpav=available providers,\n"
        << "                rp-load/rp-unload <name>, cancel/job <id>, config <key> <val>, run <args>, q=quit\n";
    ...
    while(g_state.running && std::getline(std::cin, line)) {
        ...
        if(line == "q" || line == "quit") {
            ...
            auto space_pos = rest.find(' ');
            if(space_pos == std::string::npos) {
                std::cout << "[Mock] Usage: config <key> <value>  (e.g. config maxCorrectnessWorkers 4)\n";
            } else {
            ...
            std::cout << "[Mock] Unknown command: '" << line
                << "'. Commands: s, l, qs, a, av, a-load/a-unload <name>, rp, rpav, rp-load/rp-unload <name>, cancel/job <id>, config <key> <val>, run <args>, q\n";
        }
    }
