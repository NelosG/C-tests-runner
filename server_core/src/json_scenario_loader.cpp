...
void write_json_to_test_data(TestData& td, const std::string& key, const nlohmann::json& v) {
    switch(classify(v)) {
        case JsonKind::INT:    td.write_value<long long>(key, v.get<long long>()); break;
        case JsonKind::DOUBLE: td.write_value<double>   (key, v.get<double>());    break;
        case JsonKind::BOOL:   td.write_value<bool>     (key, v.get<bool>());      break;
        case JsonKind::STRING: td.write_string          (key, v.get<std::string>()); break;
        case JsonKind::INT_ARRAY:
        case JsonKind::EMPTY_ARRAY:
            td.write_array<long long>(key, v.get<std::vector<long long>>()); break;
        case JsonKind::DOUBLE_ARRAY:
            td.write_array<double>(key, v.get<std::vector<double>>()); break;
        ...
        case JsonKind::STRING_ARRAY:
            td.write_strings(key, v.get<std::vector<std::string>>()); break;
...
std::pair<bool, std::string> compare_double_array(
    const std::string& key, const std::vector<double>& expected,
    const std::vector<double>& actual, double eps) {
    if(actual.size() != expected.size())
        return {false, "Field '" + key + "': size mismatch (expected "
...
std::pair<bool, std::string> verify_json_against_output(
    ...
    const TestData& output, double eps
) {
    if(!output.try_get(key))
    ...
    try {
        switch(classify(expected)) {
            case JsonKind::INT: {
                ...
                if(a != e) return {false, "Field '" + key + "': expected "
            ...
            case JsonKind::DOUBLE: {
                ...
                if(!doubles_equal(e, a, eps))
                    return {false, "Field '" + key + "': expected "
                        + std::to_string(e) + ", got " + std::to_string(a)};
                return {true, {}};
            }
            case JsonKind::BOOL: {
            ...
            case JsonKind::BOOL_ARRAY: {
                ...
                for(std::size_t i = 0; i < e_raw.size(); ++i) {
                    ...
                    if(actual_bit != expected_bit)
                        return {false, "Field '" + key + "': mismatch at index "
