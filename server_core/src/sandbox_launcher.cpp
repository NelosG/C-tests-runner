    ...
    const JobConfig& job_config)
{
    ...
    try {
        result = launch_isolate(box_id, runner_exe_path, input_dir, output_dir,
            thread_count, safe_mode, job_config);
    } catch (...) {
    ...
    const JobConfig& job_config)
{
    ...
    HANDLE h_nul = CreateFileA("NUL", GENERIC_READ | GENERIC_WRITE,
    ...
    std::thread stderr_reader([&stderr_output, stderr_read]() {
        char buf[4096];
        DWORD bytes_read = 0;
    ...
    const JobConfig& job_config)
{
    ...
    run_cmd << " --dir=/runner=" << shell_quote(runner_dir.string());
    run_cmd << " --dir=/input=" << shell_quote(input_dir.string());
    run_cmd << " --dir=/output=" << shell_quote(output_dir.string()) << ":rw";
    ...
    std::string ld_library_path = "/runner";
    ...
    run_cmd << " -- /runner/" << runner_filename
            << " --input /input"
            << " --output /output"
