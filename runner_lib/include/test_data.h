...
public:
    ...
    T read_value(const std::string& key) const {
        ...
        if constexpr (std::is_same_v<T, bool>) {
            auto v = read_arithmetic_array<unsigned char>(key);
            if(v.empty())
                throw std::runtime_error("TestData::read_value: '" + key + "' is empty");
            return v[0] != 0;
        } else {
            auto v = read_arithmetic_array<T>(key);
            if(v.empty())
                throw std::runtime_error("TestData::read_value: '" + key + "' is empty");
            return v[0];
        }
    ...
    // ---- Raw access (used by verify code that needs full control) ----------

    const std::vector<std::uint8_t>* try_get(const std::string& key) const {
...
private:
    ...
    const std::vector<std::uint8_t>* require(const std::string& key) const {
        auto it = entries_.find(key);
        if(it == entries_.end())
            throw std::runtime_error("TestData: missing key '" + key + "'");
        return &it->second;
    }
    ...
    template<typename T>
    std::vector<T> read_arithmetic_array(const std::string& key) const {
        const auto* blob = require(key);
        if(blob->size() % sizeof(T) != 0)
            throw std::runtime_error("TestData: '" + key + "' size not a multiple of sizeof("
