...
class ScanCorrectnessTest final : public TestScenarioExtension {
    public:
        std::vector<Test> get_tests() const override {
            return {
                {"small_1k", setup::random_array<long long>("array", 1000), verify_inclusive_scan},
                {"medium_100k", setup::random_array<long long>("array", 100000), verify_inclusive_scan},
                {"large_1e7", setup::random_array<long long>("array", 10000000), verify_inclusive_scan},
            };
        }

        std::string name() const override { return "Correctness.Scan"; }
