// Shared helpers for nearestNeighbors scenarios. Single point set:
// for each point we compare the returned k indices to a brute-force
// k-nearest-other-points reference.
#pragma once
...
namespace nn_common {
    ...
    inline verify::Fn check_knn() {
        return [](const TestData& in, const TestData& out)
            ...
            std::int64_t n = static_cast<std::int64_t>(points.size()) / dim;
            if(static_cast<std::int64_t>(result.size()) != n * k)
                return {false,
                    "result size " + std::to_string(result.size())
                    + " != n*k " + std::to_string(n * k)};
            ...
            for(std::int64_t q = 0; q < n; ++q) {
                ...
                for(std::int64_t i = 0; i < n; ++i) {
                    if(i == q) {
                        ...
                        dists[i] = sqdist(points.data() + i * dim,
                                          points.data() + q * dim, dim);
                    }
                }
                std::vector<double> sorted_d = dists;
                std::partial_sort(sorted_d.begin(),
                                  sorted_d.begin() + k,
                                  sorted_d.end());
                ...
                for(std::int64_t j = 0; j < k; ++j) {
                    ...
                    if(idx == q)
                        return {false, "q=" + std::to_string(q)
                            + " j=" + std::to_string(j)
                            + " returned self as neighbor"};
                    double d = sqdist(points.data() + idx * dim,
                                      points.data() + q * dim, dim);
                    if(d > k_th + tol)
                        return {false,
                            "q=" + std::to_string(q)
                            + " neighbor " + std::to_string(j)
                            + " idx=" + std::to_string(idx)
                            + " dist=" + std::to_string(d)
                            + " > k-th " + std::to_string(k_th)};
                }
            }
