// Vendored verbatim from pbbsbench/algorithm/suffix_array.h
// (MIT licensed, (c) 2011-2019 Guy Blelloch, Julian Shun and the PBBS team).
// This is the *doubling* suffix array used by pbbs's longestRepeatedSubstring
// /doubling bench - wrapped in a namespace so it coexists with the parallelKS
// variant. Only adjustments: angle-bracket parlay includes, namespace wrapper.
//
// A modified Apostolico/Iliopoulos/Landau/Schieber/Vishkin suffix array.
// O(n log n) worst case; for most inputs O(n) work beyond a fixed-key sort,
// depth O(log^2 n). Each round doubles the sorted substring length and drops
// suffixes already uniquely sorted.
#pragma once
